...
typedef struct __block_literal_dispatch_block_t *dispatch_block_t;

DISPATCH_EXPORT DISPATCH_RETURNS_RETAINED DISPATCH_WARN_RESULT DISPATCH_NOTHROW dispatch_data_t __attribute__((availability(ios, introduced = 5.0), availability(macos, introduced = 10.7))) dispatch_data_create(const void *buffer, size_t size, dispatch_queue_t _Nullable queue, dispatch_block_t _Nullable destructor);
...
dispatch_data_t dispatch_data_create(void *buffer, size_t size, dispatch_queue_t queue, void *pUserData, void (*pfnCallback)(void *pUserData, void *buffer))
{
    ...
    return dispatch_data_create(buffer, size, queue, &__block_literal_dispatch_block_t);
}
